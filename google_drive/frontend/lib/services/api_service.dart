...
class ApiService {
  ...
  static Future<void> deleteRecording(String recordingId) async {
    final response = await http.post(
      ...
      headers: await _getHeaders(),
      body: jsonEncode({'recordingId': recordingId}),
    );
  ...
  static Future<void> updateRecordingFileName(String recordingId, String newFileName) async {
    final response = await http.post(
  ...
  static Future<Recording?> getRecordingByEmailAndRecordingId(String recordingId) async {
    final response = await http.post(
  ...
  //noise_elimination
  static Future<Map<String, dynamic>> processRecording(String recordingId) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/recordings/process'),
        headers: await _getHeaders(),
        body: jsonEncode({
          "recordingId": recordingId,
        }),
...
// New: Process and prepare for Drive (returns processedAudio)
  static Future<Map<String, dynamic>> processAndPrepareForDrive(String recordingId) async {
    final response = await http.post(
      Uri.parse('$baseUrl/recordings/prepare_for_drive'),
      headers: await _getHeaders(),
      body: jsonEncode({'recordingId': recordingId}),
    );
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to prepare for Drive: ${response.body}');
    }
  }

// New: Update Drive info after upload
  static Future<void> updateRecordingDriveInfo(String recordingId, String driveFileId, String status) async {
    final response = await http.post(
      Uri.parse('$baseUrl/recordings/update_drive'),
      headers: await _getHeaders(),
      body: jsonEncode({
        'recordingId': recordingId,
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to update Drive info: ${response.body}');
    }
  }

// New: Increment download count
  static Future<void> incrementDownloadCount(String recordingId) async {
    final response = await http.post(
      Uri.parse('$baseUrl/recordings/increment_download'),
      headers: await _getHeaders(),
      body: jsonEncode({'recordingId': recordingId}),
    );

    if (response.statusCode != 200) {
      throw Exception('Failed to increment download count: ${response.body}');
    }
  }
}
